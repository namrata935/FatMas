...
import citytransport2
from citytransport2 import CityTransportApp
...
class mainform:
    def __init__(self, root):
        ...
        self.city_transport_app = citytransport2.CityTransportApp(self.root)

        #self.create_button()
        self.city_transport_app.open_location_page()
    ...
    '''if __name__ == "__main__":
        root = tk.Tk()
        app = citytransport2.CityTransportApp(root)
        root.mainloop()'''



root = Tk()
connection = mysql.connector.connect(host='localhost', user='root', password='Diwali2020**')

c = connection.cursor()

c.execute('create database if not exists fg;')
c.execute('use fg;')
...
create_table_query = '''
    CREATE TABLE IF NOT EXISTS users (
        ...
        vehicle_type VARCHAR(10), fasttag_amt INT DEFAULT '5000'
    )
'''
...
register_button = tk.Button(register_contentframe,text="REGISTER", font=('Verdana',16), bg='seagreen4',fg='#fff', padx=25, pady=10, width=25)
